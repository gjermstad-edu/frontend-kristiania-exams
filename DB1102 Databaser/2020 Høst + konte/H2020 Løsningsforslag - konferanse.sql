...
ALTER TABLE MåltidBestilling
 ...
 -- D) Skriv en SQL som henter ut all informasjon om deltakere som ikke er forfatter på konferansen
 SELECT * FROM deltaker WHERE DNr NOT IN(SELECT DNr from forfatter);
 
 -- E) Skriv en SQL som henter ut følgende informasjon om deltakerne: Fornavn, etternavn og hvor mange temaer de har markert som interessante.
 -- Gjør det slik at svaret listes ut med antall temaer i synkende rekkefølge.
 SELECT fornavn, etternavn, (SELECT COUNT(*) FROM deltakertema WHERE DNr = d.DNr) AS AntInteresser FROM Deltaker d
...
ORDER BY AntTemaer DESC;
 ...
 -- F) Legg inn totalt 5 måltidbestillinger
 ...
 -- I) Konferansen ligger et stykke unna en flyplass, og det kan være hensiktsmessig å tilby transport til og fra flyplassen.
 -- Opprett en eller flere tabeller som holder informasjon om deltakere som ønsker transport til eller fra flyplassen.
 -- Velg kolonner som du selv mener er hensiktmessig å benytte.
 -- Legg deretter inn to rader.
...
(1, '2020-12-16 16:15', 'Til flyplass');
 ...
 -- Viewet skal være sortert på romnummer først, deretter startidspunkt.
 -- Tidspunktet skal vises på et mer leselig format med dato først, og deretter tidspunkt (start og slutt). Eks: En presentasjon som starter 2020-11-24 09:45:00 og varer i 20 minutter skal vises: 24. november kl 09.45-10.05
 -- Fornavn og etternavn skal vises samlet. Eks: 'Hans Hansen'
 -- Ja, oppgaven er vanskelig, men fokuser på å få med alt innholdet. Deretter kan du jobbe videre med formatteringen. Noen forslag til hendige funksjoner som kan være nyttige: CONCAT, DATE_FORMAT og DATE_ADD.
 CREATE OR REPLACE VIEW PresentasjonInformasjon AS(
 SELECT r.RomNr, CONCAT(DATE_FORMAT(p.starttid, '%d. %M kl %h.%i') , '-' , DATE_FORMAT(DATE_ADD(p.startTid, INTERVAL p.varighet MINUTE),'%h.%i')) AS Tidspunkt, p.tittel, CONCAT(d.fornavn, ' ' , d.etternavn) AS Presenterer, t.TemaNavn, r.AntPlasser
 ...
 SELECT * FROM PresentasjonInformasjon;
